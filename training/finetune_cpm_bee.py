...
def finetune(
    ...
    for epoch in range(args.epoch):
        ...
        # You can use the following codes to evaluate on the dev set during training, a simple way is to select the model with the lowest loss.
        # begin dev
        # dataloader_dev = FinetuneDataset(
        #     args.dataseteval,
        #     1,
        #     1024,
        #     tokenizer,
        ...
        # dev_loss = []
        # last_data = None
        # for iteration, data in enumerate(dataloader_dev):
        ...
        # if args.save is not None and min_loss > dev_loss:
        #     bmt.print_rank("saving model checkpoint at epoch: " + str(epoch))
        #     bmt.save(model, os.path.join(args.save, args.save_name + "-best.pt"))
        #     min_loss = dev_loss

        if args.save is not None:
            bmt.print_rank("saving model checkpoint at epoch: " + str(epoch))
            bmt.save(model, os.path.join(args.save, args.save_name + "-epoch-{}.pt".format(epoch)))
